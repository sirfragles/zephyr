...
void lis2dh_test_fill(const struct emul *emul, unsigned int count)
{
	...
	bus->regs[LIS2DH_REG_FIFO_SRC] = count == 0U ? LIS2DH_FIFO_EMPTY :
		(count == 32U ? LIS2DH_FIFO_OVRN | 31U : count);
	if (count >= 16U) {
		bus->regs[LIS2DH_REG_FIFO_SRC] |= LIS2DH_FIFO_WTM;
	}
}

static int transfer(const struct emul *emul, uint8_t reg, uint8_t *bytes,
			...
			const struct spi_buf_set *tx, const struct spi_buf_set *rx)
{
	...
	return transfer(emul, cmd & 0x3fU, read ? rx->buffers[1].buf : tx->buffers[1].buf,
			len, read);
}
