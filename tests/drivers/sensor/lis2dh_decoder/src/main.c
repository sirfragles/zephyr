...
{
	uint8_t *sample = buffer + sizeof(struct lis2dh_encoded_header) +
		index * LIS2DH_ENCODED_SAMPLE_SIZE;
...
ZTEST(lis2dh_decoder, test_slow_odr_batches)
{
	uint8_t buffer[sizeof(struct lis2dh_encoded_header) +
		FIFO_SAMPLES * LIS2DH_ENCODED_SAMPLE_SIZE] = { 0 };
	uint8_t decoded[sizeof(struct sensor_three_axis_data) +
		(FIFO_SAMPLES - 1U) * sizeof(struct sensor_three_axis_sample_data)] __aligned(8);
	...
	while (fit < FIFO_SAMPLES) {
		...
		zassert_equal(data->header.base_timestamp_ns,
			      (first + 1U) * UINT64_C(1000000000));
...
ZTEST(lis2dh_decoder, test_runtime_scales)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(scales); i++) {
		...
		int64_t recovered = ((int64_t)positive * 1000000 * (INT64_C(1) << shift)) /
			(INT64_C(1) << 31);
