			    ...
			    uint8_t *value, uint8_t len)
{
	...
	if (reg_addr == LIS2DH_REG_WAI && len == 1U) {
		LOG_DBG("%s: WHO_AM_I SPI TX command=0x%02x, RX length=%u",
			dev->name, buffer_tx[0], len);
	}
...
int lis2dh_spi_init(const struct device *dev)
{
	...
	LOG_DBG("%s: SPI bus=%s frequency=%u Hz mode=%u word=%u %s first slave=%u",
		dev->name, spi->bus->name, spi->config.frequency,
	...
	if (spi_cs_is_gpio_dt(spi)) {
		LOG_DBG("%s: SPI CS GPIO=%s pin=%u active=%s",
			dev->name, spi->config.cs.gpio.port->name, spi->config.cs.gpio.pin,
