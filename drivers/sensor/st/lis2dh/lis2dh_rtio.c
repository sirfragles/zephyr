...
static int lis2dh_accel_shift(void)
{
...
static void lis2dh_submit_one_shot(struct rtio_iodev_sqe *iodev_sqe)
{
	...
	status = rtio_sqe_rx_buf(iodev_sqe, sizeof(*header) + LIS2DH_ENCODED_SAMPLE_SIZE,
				 sizeof(*header) + LIS2DH_ENCODED_SAMPLE_SIZE, &buffer,
				 &buffer_len);
	if (status < 0) {
	...
	header->trigger = 0U;
	header->shift = lis2dh_accel_shift();
	header->is_fifo = 0U;
