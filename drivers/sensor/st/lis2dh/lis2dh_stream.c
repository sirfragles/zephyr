...
static int lis2dh_stream_arm(const struct device *dev,
			     const struct sensor_read_config *cfg)
{
	...
	status = data->hw_tf->update_reg(dev, LIS2DH_REG_CTRL3,
...
int lis2dh_stream_handle_irq(const struct device *dev)
{
	...
	for (size_t i = 0U; i < cfg->count; i++) {
		const struct sensor_stream_trigger *trig = &cfg->triggers[i];
		uint8_t flag = trig->trigger == SENSOR_TRIG_FIFO_FULL ?
			LIS2DH_FIFO_OVRN : LIS2DH_FIFO_WTM;
	...
	count = (src & LIS2DH_FIFO_EMPTY) != 0U ? 0U :
	...
	if (count != 0U) {
		...
		for (size_t axis = 0U; axis < 3U; axis++) {
			data->sample.xyz[axis] = (int16_t)sys_get_le16(buffer + sizeof(*header) +
